...
<template>
  <Dialog v-model:open="open" :title="`預覽 · ${datasetKey}`" width-class="max-w-3xl">
    <div class="mt-3">
      <div v-if="loading" class="flex items-center gap-2 py-8 text-sm text-muted-foreground">
        <Loader2 class="size-4 animate-spin" />載入資料中…（首次預覽需建立快取，可能較久）
      </div>
      ...
      <Tabs v-else-if="data" v-model="tab" class="space-y-3">
        <TabsList>
          ...
          <TabsTrigger value="samples">範例資料（{{ data.rows.length }}）</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="intro" class="space-y-3">
          <div class="flex flex-wrap items-center gap-1.5">
            ...
            <span v-if="(data.subsets?.length ?? 0) > 1" class="text-[10px] text-muted-foreground">· {{ data.subsets!.length }} 個主題</span>
          </div>
          <div v-if="descHtml" class="markdown max-h-[60vh] overflow-y-auto text-sm" v-html="descHtml" />
          <p v-else class="text-xs text-muted-foreground">此資料集沒有提供介紹。</p>
        </TabsContent>
        ...
        <TabsContent value="samples" class="space-y-3">
          <p class="text-xs text-muted-foreground">
            ...
            ·
            <template v-if="data.truncated">顯示前 <span class="font-mono text-foreground">{{ data.rows.length }}</span> 筆（資料集更大）</template>
            <template v-else>共 <span class="font-mono text-foreground">{{ data.count }}</span> 筆</template>
          </p>
          <div class="max-h-[60vh] space-y-2 overflow-y-auto">
            ...
            >
              <div class="mb-1 flex items-center gap-2">
                ...
                <span v-if="r.target != null" class="text-[10px] text-[var(--chart-1)]">答案：<span class="font-mono">{{ r.target }}</span></span>
              </div>
