...
import time
import os
from app.launcher.llm_launcher import launch_single_llm_model, stop_single_llm_model
...
def start_llm_model(model_name: str, request: Request):
    ...
    app = request.app
    app.state.starting_models.add(model_name)
    try:
        launch_single_llm_model(model_name, config_path)
        ...
        app.state.starting_models.discard(model_name)

@router.post("/llm/stop/{model_name}")
def stop_llm_model(model_name: str):
    try:
        stop_single_llm_model(model_name)
